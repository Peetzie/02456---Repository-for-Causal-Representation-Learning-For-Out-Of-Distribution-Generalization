...
def prepare():
    """
        Prepare working directories and label files!
    """

    Path("dataset/CMNIST/Train1/").mkdir(parents=True, exist_ok=True)
    Path("dataset/CMNIST/Train2/").mkdir(parents=True, exist_ok=True)
    Path("dataset/CMNIST/Test/").mkdir(parents=True, exist_ok=True)
    Path("data/ColoredMNIST").mkdir(parents=True, exist_ok=True)

    file1 = Path('dataset/CMNIST/Train1/labels.txt')
    file1.touch(exist_ok=True)
    file2 = Path('dataset/CMNIST/Train2/labels.txt')
    file2.touch(exist_ok=True)
    file3 = Path('dataset/CMNIST/Test/labels.txt')
        ...
        if env in ['train1', 'train2', 'test']:
            ...
            self.data_label_tuples = torch.load(os.path.join(self.root, 'ColoredMNIST', 'train1.pt')) + \
                torch.load(os.path.join(
                    self.root, 'ColoredMNIST', 'train2.pt'))
        else:
            raise RuntimeError(
    ...
    def prepare_colored_mnist(self, noOfEnviroments = 2):
        ...
        for idx, (im, label) in enumerate(train_mnist):
            ...
            if idx < 20000:
                ...
                im.save("dataset/CMNIST/Train1/{}.jpg".format(idx))
                ...
                im = Image.fromarray(colored_arr)
                im.save("dataset/CMNIST/Test/{}.jpg".format(idx))
                with open("dataset/CMNIST/Test/labels.txt", "a") as file_object:
                    file_object.write(str(idx) + ".jpg" +
                                      ", " + str(binary_label) + "\n")
